...
export function HeroGlobe({ className = "", variant = "location" }: {
  className?: string;
  ...
  return (
    <div className={`relative pointer-events-none select-none ${className}`}>
      <svg
        ...
        aria-hidden
        className="overflow-visible animate-spin-slow"
      ...
      {engineer ? (
        <span className="absolute inset-0 grid place-items-center font-mono text-[10px] font-semibold uppercase tracking-[0.12em] text-accent-soft [text-shadow:0_0_8px_#0d0f14,0_0_14px_#0d0f14]">
          Engineer
        </span>
      ) : (
